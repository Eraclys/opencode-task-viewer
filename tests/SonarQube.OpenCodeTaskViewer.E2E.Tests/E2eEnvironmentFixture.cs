...
public sealed class E2eEnvironmentFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var mockProjectPath = Path.Combine(
            RootDir,
            "tests",
            "SonarQube.OpenCodeTaskViewer.MockOpenCode",
            "SonarQube.OpenCodeTaskViewer.MockOpenCode.csproj");

        var mockSonarProjectPath = Path.Combine(
            RootDir,
            "tests",
            "SonarQube.OpenCodeTaskViewer.MockSonarQube",
            "SonarQube.OpenCodeTaskViewer.MockSonarQube.csproj");

        var viewerProjectPath = Path.Combine(
            RootDir,
            "src",
            "SonarQube.OpenCodeTaskViewer.Server",
            "SonarQube.OpenCodeTaskViewer.Server.csproj");
    ...
    static string FindRootDir()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir, "SonarQube.OpenCodeTaskViewer.slnx")))
                return dir;
