...
public sealed class OpenCodeEventHandlerTests
{
    ...
    public async Task HandleAsync_TodoUpdatedForSession_InvalidatesTodoCacheAndBroadcastsSessionUpdate()
    {
        ...
        viewerState.StoreTodos(
            "C:/Work",
            "sess-1",
            [new SessionTodoDto("todo-1", ViewerTaskStatus.FromRaw("open"), "task")],
            cachePolicy.TodoCacheTtlMs);

        using var responseBody = new MemoryStream();
        var (hub, client, cts) = CreateSseHub(responseBody);

        var sut = new OpenCodeEventHandler(
            viewerState,
            hub,
            new OpenCodeCacheInvalidationPolicy(),
            cachePolicy);
