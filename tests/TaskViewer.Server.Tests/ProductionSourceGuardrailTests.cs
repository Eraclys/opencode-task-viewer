...
public sealed class ProductionSourceGuardrailTests
{
    ...
    static string FindProductionSourceRoot()
    {
        ...
        {
            var candidate = Path.Combine(directory.FullName, "src", "TaskViewer.Server");
        ...
        throw new InvalidOperationException("Could not locate src/TaskViewer.Server from test output directory.");
    }
        ...
        for (var index = 0; index < source.Length; index++)
        {
            ...
            switch (state)
            {
                case TokenState.Code:
                    if (TryStartRawString(source, index, out var rawPrefixLength, out rawStringQuoteCount))
                    {
                        AppendSanitized(builder, source, index, rawPrefixLength);
                    ...
                    if (current == '/' && next == '/')
                    {
                        AppendSanitized(builder, source, index, 2);
                        index++;
                    ...
                    {
                        AppendSanitized(builder, source, index, verbatimPrefixLength);
                    ...
                    {
                        AppendSanitized(builder, source, index, stringPrefixLength);
                ...
                case TokenState.RawString:
                    if (current == '"' && CountConsecutive(source, index, '"') >= rawStringQuoteCount)
                    {
                        AppendSanitized(builder, source, index, rawStringQuoteCount);
    ...
    static bool TryStartRawString(string source, int index, out int prefixLength, out int quoteCount)
    {
        var cursor = index;
    ...
    static bool IsVerbatimStringStart(string source, int index, out int prefixLength)
    {
        ...
        if (Matches(source, index, "@\"") || Matches(source, index, "$@\"") || Matches(source, index, "@$\""))
        {
    ...
    static void AppendSanitized(StringBuilder builder, string source, int start, int length)
    {
