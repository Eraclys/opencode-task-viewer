...
public sealed class QueueCommandsServiceTests
{
    ...
    private sealed class FakeQueueRepository : IQueueRepository
    {
        ...
        public Task<QueueItemRecord?> ClaimNextQueuedItem(string now)
            => throw new NotSupportedException();
        public Task<bool> MarkSessionCreated(int id, string sessionId, string? openCodeUrl, string timestamp)
            => throw new NotSupportedException();
        public Task<(int AttemptCount, int MaxAttempts)> GetAttemptInfo(int id, int fallbackAttemptCount, int fallbackMaxAttempts)
            => throw new NotSupportedException();
        public Task<bool> MarkDispatchFailure(int id, string state, string? nextAttemptAt, string lastError, string updatedAt)
            => throw new NotSupportedException();
    }
}
