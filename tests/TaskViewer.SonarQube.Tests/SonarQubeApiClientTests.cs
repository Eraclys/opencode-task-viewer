...
public sealed class SonarQubeApiClientTests
{
    [Fact]
    ...
    static SonarQubeApiClient CreateGateway(HttpMessageHandler handler)
        => new(() => new SonarQubeTypedHttpClient(new HttpClient(handler), new SonarQubeClientOptions("http://sonar.local", "secret")));
}
