...
app.MapPost("/__test__/setTodos", async (HttpRequest request) =>
{
    var body = await ReadBody(request);
    var sessionId = body?["sessionId"]?.GetValue<string>();
    var todos = body?["todos"] as JsonArray;
    if (string.IsNullOrWhiteSpace(sessionId) || todos is null)
        return Results.Json(new { error = "Expected { sessionId, todos: [] }" }, statusCode: 400);
...
app.MapPost("/__test__/setStatus", async (HttpRequest request) =>
{
    var body = await ReadBody(request);
    var directory = NormalizeDir(body?["directory"]?.GetValue<string>());
    var sessionId = body?["sessionId"]?.GetValue<string>();
    var type = body?["type"]?.GetValue<string>();
    if (string.IsNullOrWhiteSpace(directory) || string.IsNullOrWhiteSpace(sessionId) || string.IsNullOrWhiteSpace(type))
        return Results.Json(new { error = "Expected { directory, sessionId, type }" }, statusCode: 400);
    ...
    lock (gate)
    {
        ...
        statuses[sessionId] = new JsonObject { ["type"] = type };
    }

    return Results.Json(new { ok = true });
});

app.MapPost("/__test__/emit", async (HttpRequest request) =>
{
    var body = await ReadBody(request);
    var directory = body?["directory"]?.GetValue<string>();
    var type = body?["type"]?.GetValue<string>();
    var properties = body?["properties"] as JsonObject ?? new JsonObject();
    if (string.IsNullOrWhiteSpace(directory) || string.IsNullOrWhiteSpace(type))
        return Results.Json(new { error = "Expected { directory, type, properties? }" }, statusCode: 400);

    await Broadcast(new JsonObject
    {
        ["directory"] = directory,
...
app.MapPost("/__test__/setFailures", async (HttpRequest request) =>
{
    var body = await ReadBody(request);
    lock (gate)
    {
...
app.MapPost("/__test__/addSandboxSession", async (HttpRequest request) =>
{
    var body = await ReadBody(request);
    ...
    if (string.IsNullOrWhiteSpace(projectWorktree) || string.IsNullOrWhiteSpace(sandboxPath) || string.IsNullOrWhiteSpace(directory))
        return Results.Json(new { error = "Expected { projectWorktree, sandboxPath, directory? }" }, statusCode: 400);
    ...
    lock (gate)
    {
        ...
        if (project is null)
        {
            project = new ProjectRecord
            {
                ...
                Time = new TimeRecord { Created = now, Updated = now }
            };
        ...
        {
            state.Sessions.Insert(0, new SessionRecord
            {
                Id = sessionId,
                Title = title,
                Directory = directory,
                Project = new SessionProjectRecord { Worktree = projectWorktree },
                Time = new TimeRecord { Created = now, Updated = now }
            });
        }
            ...
            [
                new MessageRecord
                {
                    Info = new MessageInfoRecord
                    {
                        Id = $"m-{sessionId}",
                        Role = "user",
                        Time = new MessageTimeRecord { Created = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() }
                    },
    ...
    return Results.Json(new { ok = true, sessionId, directory, projectWorktree, sandboxPath });
});
...
app.MapGet("/session", (HttpRequest request) =>
{
...
app.MapPost("/session", async (HttpRequest request) =>
{
    lock (gate)
    {
        if (state.FailSessionCreateCount > 0)
        {
            ...
            return Results.Json(new { error = "Injected session creation failure" }, statusCode: 500);
        }
    }
    ...
    if (string.IsNullOrWhiteSpace(directory))
        return Results.Json(new { error = "Missing directory" }, statusCode: 400);
    ...
    lock (gate)
    {
        ...
        created = new SessionRecord
        {
            ...
            Title = title,
            Directory = directory,
            Project = new SessionProjectRecord { Worktree = worktree },
            Time = new TimeRecord { Created = now, Updated = now }
        };
        ...
        {
            state.Projects.Add(new ProjectRecord
            {
                Id = $"p-{sessionId}",
                Worktree = worktree,
                Time = new TimeRecord { Created = now, Updated = now }
    ...
    _ = Broadcast(new JsonObject
    {
        ["directory"] = directory,
        ["payload"] = new JsonObject
        {
            ["type"] = "session.created",
            ["properties"] = new JsonObject { ["sessionID"] = created.Id }
        }
    });
...
app.MapGet("/experimental/session", (HttpRequest request) =>
{
...
app.MapGet("/session/status", (HttpRequest request) =>
{
    var directory = NormalizeDir(request.Query["directory"].ToString());
    if (string.IsNullOrWhiteSpace(directory))
        return Results.Json(new { error = "Missing directory" }, statusCode: 400);
...
app.MapGet("/session/{sessionId}", (string sessionId) =>
{
    lock (gate)
    {
...
app.MapGet("/session/{sessionId}/todo", (string sessionId) =>
{
    lock (gate)
    {
...
app.MapPatch("/session/{sessionId}", async (string sessionId, HttpRequest request) =>
{
    var body = await ReadBody(request);
    ...
    lock (gate)
    {
        ...
        if (body?["time"] is JsonObject timeObj && timeObj["archived"] is not null)
        {
...
app.MapGet("/session/{sessionId}/message", (string sessionId, HttpRequest request) =>
{
    var limitRaw = request.Query["limit"].ToString();
...
app.MapPost("/session/{sessionId}/prompt_async", async (string sessionId, HttpRequest request) =>
{
    ...
    lock (gate)
    {
        ...
        if (state.FailPromptAsyncCount > 0)
        {
            ...
            return Results.Json(new { error = "Injected prompt_async failure" }, statusCode: 500);
        }
    ...
    lock (gate)
    {
        ...
        statuses[sessionId] = new JsonObject { ["type"] = "busy" };
        ...
        messages.Add(new MessageRecord
        {
            Info = new MessageInfoRecord
            {
                ...
                Role = "user",
                Time = new MessageTimeRecord { Created = nowMs }
            },
    ...
    await Broadcast(new JsonObject
    {
        ...
        ["payload"] = new JsonObject
        {
            ...
            {
                ["sessionID"] = sessionId,
                ["status"] = new JsonObject { ["type"] = "busy" }
            }
        }
    });
...
app.MapGet("/global/event", async (HttpContext context) =>
{
...
static int ParsePositiveInt(JsonNode? node, int fallback)
{
    ...
    if (!int.TryParse(node.ToString(), out var parsed) || parsed <= 0)
        return fallback;
        ...
        var state = new MockState
        {
            ...
            [
                new ProjectRecord
                {
                    Id = "global",
                    Worktree = "/",
                    Time = new TimeRecord { Created = baseCreated, Updated = now.AddSeconds(-5).ToString("O") }
                },
                new ProjectRecord
                {
                    Id = "p-alpha",
            ...
            [
                new SessionRecord
                {
                    ...
                    Title = "Busy Session",
                    Directory = betaDir,
                    Project = new SessionProjectRecord { Worktree = betaWorktree },
                    Time = new TimeRecord { Created = baseCreated, Updated = now.AddSeconds(-20).ToString("O") }
                },
                new SessionRecord
                {
                    Id = "sess-retry",
                    ...
                    Directory = alphaDir,
                    Project = new SessionProjectRecord { Worktree = alphaWorktree },
                    Time = new TimeRecord { Created = baseCreated, Updated = now.AddSeconds(-45).ToString("O") }
                },
                new SessionRecord
                {
                    Id = "sess-recent",
                    ...
                    Directory = gammaDir,
                    Project = new SessionProjectRecord { Worktree = gammaWorktree },
                    Time = new TimeRecord { Created = baseCreated, Updated = now.AddMinutes(-2).ToString("O") }
                },
                new SessionRecord
                {
                    Id = "sess-stale",
                    ...
                    Directory = gammaDir,
                    Project = new SessionProjectRecord { Worktree = gammaWorktree },
                    Time = new TimeRecord { Created = baseCreated, Updated = now.AddMinutes(-10).ToString("O") }
                },
                new SessionRecord
                {
                    Id = "sess-archived",
            ...
            {
                ["sess-busy"] = new JsonArray(),
                ["sess-retry"] = new JsonArray(),
                ["sess-recent"] = new JsonArray(),
                ["sess-stale"] = new JsonArray()
            },
                ...
                [
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m1",
                            Role = "user",
                            Time = new MessageTimeRecord { Created = now.AddSeconds(-30).ToUnixTimeMilliseconds() }
                        },
                        Content =
                        [
                            new MessageContentRecord { Type = "text", Text = "Run the worker." }
                        ]
                    },
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m2",
                            Role = "assistant",
                            Time = new MessageTimeRecord { Created = now.AddSeconds(-29).ToUnixTimeMilliseconds() }
                        },
                        Content =
                        [
                            new MessageContentRecord { Type = "text", Text = "Worker is running now." }
                        ]
                    }
                ],
                ["sess-retry"] =
                [
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m3",
                            Role = "user",
                            Time = new MessageTimeRecord { Created = now.AddSeconds(-60).ToUnixTimeMilliseconds() }
                        },
                    ...
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m4",
                            Role = "assistant",
                            Time = new MessageTimeRecord { Created = now.AddSeconds(-59).ToUnixTimeMilliseconds() }
                        },
                ...
                [
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m5",
                            Role = "user",
                            Time = new MessageTimeRecord { Created = now.AddMinutes(-2).ToUnixTimeMilliseconds() }
                        },
                ...
                [
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m6",
                            Role = "user",
                            Time = new MessageTimeRecord { Created = now.AddHours(-1).ToUnixTimeMilliseconds() }
                        },
                    ...
                    new MessageRecord
                    {
                        Info = new MessageInfoRecord
                        {
                            Id = "m7",
                            Role = "assistant",
                            Time = new MessageTimeRecord { Created = now.AddHours(-1).AddSeconds(1).ToUnixTimeMilliseconds() }
                        },
            ...
            {
                [alphaDir] = new Dictionary<string, JsonObject>(StringComparer.OrdinalIgnoreCase)
                {
                    ["sess-retry"] = new JsonObject { ["type"] = "retry" }
                },
                [betaDir] = new Dictionary<string, JsonObject>(StringComparer.OrdinalIgnoreCase)
                {
                    ["sess-busy"] = new JsonObject { ["type"] = "busy" }
                },
                [gammaDir] = new Dictionary<string, JsonObject>(StringComparer.OrdinalIgnoreCase)
            }
        };
