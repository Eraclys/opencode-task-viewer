...
public sealed class OpenCodeViewerState
{
    ...
    {
        if (!_cache.TryGetValue<List<OpenCodeProject>>(ProjectsCacheKey, out var projects) ||
            projects is null ||
            projects.Count == 0)
            return null;
    ...
    {
        if (_cache.TryGetValue<Dictionary<string, SessionRuntimeStatus>>(StatusMapCacheKey(directoryKey), out var cached) &&
            cached is not null)
        {
    ...
    public bool TryGetFreshTodos(string? directory, string sessionId, out List<SessionTodoDto> todos)
    {
        ...
        if (_cache.TryGetValue<List<SessionTodoDto>>(cacheKey, out var cached) &&
            cached is not null)
        {
            todos = [.. cached];
    ...
    {
        if (_cache.TryGetValue<List<OpenCodeSessionDto>>(DirectorySessionsCacheKey(directoryKey), out var cached) &&
            cached is not null)
        {
            sessions = [.. cached];
    ...
    {
        if (!_cache.TryGetValue<List<GlobalViewerTaskDto>>(TasksAllCacheKey, out var tasks) ||
            tasks is null ||
            tasks.Count == 0)
            return null;
    ...
    public void NoteStatusOverride(
        string? directory,
        string sessionId,
        SessionRuntimeStatus type,
        int ttlMs)
    {
