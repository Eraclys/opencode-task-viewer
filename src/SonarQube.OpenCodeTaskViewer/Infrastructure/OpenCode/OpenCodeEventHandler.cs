...
public sealed class OpenCodeEventHandler
{
    ...
    public OpenCodeEventHandler(
        OpenCodeViewerState viewerState,
        ISseHub sseHub,
        OpenCodeCacheInvalidationPolicy cacheInvalidationPolicy,
    ...
    public async Task HandleAsync(OpenCodeSseEvent evt)
    {
        ...
        if (decision.InvalidateSessionTodos &&
        ...
        if (!string.IsNullOrWhiteSpace(parsed.SessionId) &&
            decision.StatusType is { } statusType)
            _viewerState.NoteStatusOverride(
                decision.StatusDirectory,
                parsed.SessionId,
                statusType,
                _cachePolicy.StatusOverrideTtlMs);
