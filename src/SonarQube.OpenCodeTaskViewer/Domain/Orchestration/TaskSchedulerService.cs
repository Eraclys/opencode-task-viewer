...
sealed class TaskSchedulerService : ITaskSchedulerService
{
    ...
    public async Task<QueueItemRecord?> LeaseNextTaskAsync(
        string leaseOwner,
        int globalMaxActive,
        ...
        int leaseSeconds)
    {
        ...
        var active = await _queueRepository.ListQueue(
            [
                QueueState.Leased,
                QueueState.Running
            ],
        ...
        return await _queueRepository.TryLeaseTask(
            candidate.Id,
            leaseOwner,
            heartbeatAt,
            expiresAt);
    ...
    static string GetProjectKey(QueueItemRecord task) => $"{task.SonarProjectKey}::{task.Branch ?? string.Empty}";
    ...
    static string NormalizeLockKey(string? value) => (value ?? string.Empty).Trim();
}
