...
sealed class TaskReadinessGate : ITaskReadinessGate
{
    ...
    static bool HasRepositoryConflict(string directory)
    {
        ...
        return File.Exists(Path.Combine(gitDir, "MERGE_HEAD")) || Directory.Exists(Path.Combine(gitDir, "rebase-merge")) || Directory.Exists(Path.Combine(gitDir, "rebase-apply"));
    }
}
