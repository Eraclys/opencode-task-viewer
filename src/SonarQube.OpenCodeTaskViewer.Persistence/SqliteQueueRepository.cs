...
SELECT last_insert_rowid();",
                    ...
                    {
                        TaskKey = taskKey,
                        OrchestrationTaskBatchingPolicy.TaskUnit,
                        IssueKey = issueKey,
...
INSERT INTO task_issue_links (
    ...
    @RelativePath, @AbsolutePath, @Line, @IssueStatus, @CreatedAt
            ...
            var inserted = await conn.QuerySingleOrDefaultAsync<QueueItemRow>(
                Cmd(
                    SelectTaskByIdSql,
                    new
                    {
...
WHERE id = @Id AND state IN ('queued', 'leased', 'running')",
                new
                {
                    Now = nowIso,
                    Id = id
...
WHERE state = 'queued'",
    ...
    public async Task<QueueItemRecord?> TryLeaseTask(
        int id,
        string leaseOwner,
...
WHERE id = @Id AND state = 'queued'",
        ...
        var row = await conn.QuerySingleOrDefaultAsync<QueueItemRow>(
            SelectTaskByIdSql,
            new
            {
                Id = id
    ...
    public async Task<bool> HeartbeatTask(
        int id,
        string leaseOwner,
        DateTimeOffset heartbeatAt,
        DateTimeOffset expiresAt,
...
WHERE id = @Id AND lease_owner = @LeaseOwner AND state IN ('leased', 'running')",
            new
            {
                HeartbeatAt = heartbeatAt.ToString("O"),
                ExpiresAt = expiresAt.ToString("O"),
...
ORDER BY issue_key ASC",
        ...
        return rows
            .Select(row => new NormalizedIssue
            {
                Key = row.IssueKey,
                Type = row.IssueType,
...
WHERE id = @Id AND state = 'running'",
    ...
    public async Task<bool> ApproveTask(int id, DateTimeOffset timestamp, CancellationToken cancellationToken = default) => await UpdateTerminalReviewState(
        id,
        QueueState.Done,
        TaskReviewAction.Approved,
        null,
        timestamp,
        [QueueState.AwaitingReview]);

    public async Task<bool> RejectTask(
        int id,
        string? reason,
        DateTimeOffset timestamp,
        CancellationToken cancellationToken = default) => await UpdateTerminalReviewState(
        id,
        QueueState.Rejected,
        TaskReviewAction.Rejected,
        reason,
        timestamp,
        [QueueState.AwaitingReview]);

    public async Task<bool> RequeueTask(
        int id,
        string? reason,
        DateTimeOffset timestamp,
        CancellationToken cancellationToken = default)
...
SET state = 'queued',
    ...
    session_id = NULL,
    open_code_url = NULL
WHERE id = @Id AND state IN ('awaiting_review', 'rejected')",
            new
            {
                Id = id,
                Timestamp = timestamp.ToString("O"),
        ...
        if (changed > 0)
        {
            await AppendReviewHistoryAsync(
                conn,
                tx,
                id,
                TaskReviewAction.Requeue,
                reason,
                timestamp);
...
WHERE id = @Id AND state IN ('awaiting_review', 'rejected')",
    ...
    public async Task<(int AttemptCount, int MaxAttempts)> GetAttemptInfo(
        int id,
...
WHERE id = @Id AND state IN ('leased', 'running')",
    ...
    async Task<bool> UpdateTerminalReviewState(
        int id,
        QueueState state,
        TaskReviewAction action,
        string? reason,
        DateTimeOffset timestamp,
        IReadOnlyList<QueueState> allowedStates)
    {
        using var conn = _openConnection();
        using var tx = conn.BeginTransaction();
...
SET state = @State,
    ...
    lease_heartbeat_at = NULL,
    lease_expires_at = NULL
WHERE id = @Id AND state IN @AllowedStates",
            new
            {
                Id = id,
                State = state.Value,
    ...
    async Task AppendReviewHistoryAsync(
...
VALUES (@TaskId, @Action, @Reason, @CreatedAt)",
            new
            {
                TaskId = taskId,
                Action = action.Value,
