...
public interface IQueueRepository
{
    ...
    Task<QueueItemRecord?> TryLeaseTask(int id, string leaseOwner, DateTimeOffset heartbeatAt, DateTimeOffset expiresAt, CancellationToken cancellationToken = default);
    Task<bool> HeartbeatTask(int id, string leaseOwner, DateTimeOffset heartbeatAt, DateTimeOffset expiresAt, CancellationToken cancellationToken = default);
    ...
    Task<bool> RejectTask(int id, string? reason, DateTimeOffset timestamp, CancellationToken cancellationToken = default);
    Task<bool> RequeueTask(int id, string? reason, DateTimeOffset timestamp, CancellationToken cancellationToken = default);
    Task<bool> RepromptTask(int id, string instructions, string? reason, DateTimeOffset timestamp, CancellationToken cancellationToken = default);

    Task<(int AttemptCount, int MaxAttempts)> GetAttemptInfo(int id, int fallbackAttemptCount, int fallbackMaxAttempts, CancellationToken cancellationToken = default);
