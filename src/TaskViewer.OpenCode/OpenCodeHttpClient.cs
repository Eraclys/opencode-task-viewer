...
public sealed class OpenCodeHttpClient
{
        ...
        var attempts = new Func<Task>[]
        {
            () => SendAsync(
                ...
                new OpenCodeRequest
                {
                    Method = "PATCH",
                    Directory = directory,
                    JsonBody = JsonSerializer.Serialize(new { time = new { archived = nowUnixMs } })
                },
                cancellationToken),
            () => SendAsync(
                $"/session/{Uri.EscapeDataString(sessionId)}",
                new OpenCodeRequest
                {
                    Method = "PATCH",
                    Directory = directory,
                    JsonBody = JsonSerializer.Serialize(new { archived = true })
                },
                cancellationToken),
            () => SendAsync(
    ...
    static OpenCodeMessage ParseMessage(OpenCodeMessageTransport? message)
    {
    ...
    static string ExtractObjectTextFragment(JsonElement value, int depth)
    {
        ...
        if (value.TryGetProperty("parts", out var parts) && parts.ValueKind == JsonValueKind.Array)
        {
    ...
    sealed class OpenCodeArrayEnvelope<T>
    {
        ...
        public List<T>? Sessions { get; init; }
    ...
    sealed class OpenCodeMessageTransport
    {
        ...
        public OpenCodeMessageInfoTransport? Info { get; init; }

        [JsonPropertyName("role")]
        public string? Role { get; init; }

        [JsonPropertyName("author")]
        public OpenCodeMessageAuthorTransport? Author { get; init; }
        ...
        public JsonElement? Parts { get; init; }

        [JsonPropertyName("data")]
        public JsonElement? Data { get; init; }

        [JsonPropertyName("time")]
        public OpenCodeTimeTransport? Time { get; init; }
    ...
    sealed class OpenCodeProjectTransport
    {
        ...
        public JsonElement? Sandboxes { get; init; }
    }
    ...
    sealed class OpenCodeSessionTransport
    {
        ...
        public OpenCodeProjectTransport? Project { get; init; }
    ...
    sealed class OpenCodeTimeTransport
    {
        ...
        public JsonElement? Archived { get; init; }
    }
