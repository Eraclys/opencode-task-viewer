...
public static class AssistantMessageParser
{
    ...
    {
        return (message?["info"]?["role"]?.ToString() ?? message?["role"]?.ToString() ?? message?["author"]?["role"]?.ToString() ?? string.Empty)
            .Trim()
            .ToLowerInvariant();
    }
    ...
    static string ExtractTextFragment(JsonNode? value, int depth = 0)
    {
        ...
        switch (value)
        {
            case JsonValue jsonValue: return jsonValue.ToString().Trim();
