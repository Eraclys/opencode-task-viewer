...
var orchestratorDbPath = Environment.GetEnvironmentVariable("ORCHESTRATOR_DB_PATH")
  ?? Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "data", "orchestrator.sqlite");

var orchMaxActive = ParseIntSafe(Environment.GetEnvironmentVariable("ORCH_MAX_ACTIVE"), 3, min: 1);
var orchPollMs = ParseIntSafe(Environment.GetEnvironmentVariable("ORCH_POLL_MS"), 3000, min: 1000);
var orchMaxAttempts = ParseIntSafe(Environment.GetEnvironmentVariable("ORCH_MAX_ATTEMPTS"), 3, min: 1);
var orchMaxWorkingGlobal = ParseIntSafe(Environment.GetEnvironmentVariable("ORCH_MAX_WORKING_GLOBAL"), 5, min: 0);
var resumeFallback = orchMaxWorkingGlobal > 1 ? orchMaxWorkingGlobal - 1 : orchMaxWorkingGlobal;
var orchWorkingResumeBelow = ParseIntSafe(Environment.GetEnvironmentVariable("ORCH_WORKING_RESUME_BELOW"), resumeFallback, min: 0);
if (orchMaxWorkingGlobal > 0 && orchWorkingResumeBelow >= orchMaxWorkingGlobal)
{
...
async Task<JsonNode?> OpenCodeFetch(string endpointPath, OpenCodeRequest req)
{
  ...
  if (!string.IsNullOrWhiteSpace(existingQuery)) queryParts.Insert(0, existingQuery);
...
var orchestrator = new SonarOrchestrator(new SonarOrchestratorOptions
{
  SonarUrl = sonarUrl,
  ...
  {
    InvalidateAllCaches();
    _ = sseHub.Broadcast(new { type = "update" });
  }
});
...
app.MapGet("/api/sessions", async (HttpContext ctx) =>
{
  ...
  try
  {
    ...
    if (string.IsNullOrWhiteSpace(limitParam)) limitParam = "20";
    ...
    foreach (var session in globalSessions)
    {
      ...
      if (!directoriesByKey.ContainsKey(key)) directoriesByKey[key] = dir!;
    }
    ...
    var tasks = globalSessions.Select(async session =>
    {
      ...
      try
      {
        ...
        var hasAssistantResponse = IsRuntimeRunning(runtimeStatus) ? (bool?)null : await GetHasAssistantResponse(sessionId);
...
app.MapGet("/api/sessions/{sessionId}", async (string sessionId) =>
{
  try
  {
    ...
    if (info is null) return Results.Json(new { error = "Session not found" }, statusCode: 404);
...
app.MapGet("/api/sessions/{sessionId}/last-assistant-message", async (string sessionId) =>
{
  try
  {
    var info = await FindSessionInfo(sessionId);
...
app.MapPost("/api/sessions/{sessionId}/archive", async (string sessionId) =>
{
  try
  {
    var info = await FindSessionInfo(sessionId);
    ...
    InvalidateAllCaches();
    await sseHub.Broadcast(new { type = "update" });
    return Results.Json(new { ok = true, archivedAt });
  }
  catch (Exception error)
  {
...
app.MapGet("/api/orch/mappings", async () =>
{
  try
  {
...
app.MapPost("/api/orch/mappings", async (HttpContext ctx) =>
{
  try
  {
  ...
  catch (Exception error)
  {
    ...
    return Results.Json(new { error = msg }, statusCode: status);
  }
});

app.MapGet("/api/orch/instructions", async (HttpContext ctx) =>
{
  try
  {
...
app.MapPost("/api/orch/instructions", async (HttpContext ctx) =>
{
  try
  {
    var body = await JsonNode.ParseAsync(ctx.Request.Body);
...
app.MapGet("/api/orch/issues", async (HttpContext ctx) =>
{
  SetNoStore(ctx.Response);
  try
  {
    var mappingId = ctx.Request.Query["mappingId"].ToString();
    if (string.IsNullOrWhiteSpace(mappingId)) return Results.Json(new { error = "Missing mappingId" }, statusCode: 400);

    var result = await orchestrator.ListIssues(mappingId,
...
app.MapGet("/api/orch/rules", async (HttpContext ctx) =>
{
  SetNoStore(ctx.Response);
  try
  {
    ...
    var result = await orchestrator.ListRules(mappingId,
      ctx.Request.Query["issueType"].ToString(),
      ctx.Request.Query["issueStatus"].ToString());
    return Results.Json(result);
  }
...
app.MapPost("/api/orch/enqueue", async (HttpContext ctx) =>
{
  try
  {
    var body = await JsonNode.ParseAsync(ctx.Request.Body);
    var issues = body?["issues"] as JsonArray;
    var result = await orchestrator.EnqueueIssues(body?["mappingId"]?.ToString(), body?["issueType"]?.ToString(), body?["instructions"]?.ToString(), issues);
    return Results.Json(result);
  }
  catch (Exception error)
  {
    var msg = error.Message;
    var status = msg.Contains("Missing", StringComparison.OrdinalIgnoreCase)
      || msg.Contains("No issues", StringComparison.OrdinalIgnoreCase)
      || msg.Contains("Mapping not found", StringComparison.OrdinalIgnoreCase)
      ? 400 : 502;
...
app.MapPost("/api/orch/enqueue-all", async (HttpContext ctx) =>
{
  try
  {
    var body = await JsonNode.ParseAsync(ctx.Request.Body);
  ...
  {
    var msg = error.Message;
    var status = msg.Contains("required", StringComparison.OrdinalIgnoreCase)
...
app.MapGet("/api/orch/queue", async (HttpContext ctx) =>
{
  SetNoStore(ctx.Response);
  try
  {
    ...
    return Results.Json(new { items, stats, worker });
  }
  catch (Exception error)
  {
...
app.MapPost("/api/orch/queue/{queueId}/cancel", async (string queueId) =>
{
  try
  {
    var ok = await orchestrator.CancelQueueItem(queueId);
    if (!ok) return Results.Json(new { error = "Queue item not found or already terminal" }, statusCode: 404);
...
app.MapPost("/api/orch/queue/retry-failed", async () =>
{
  try
  {
...
app.MapPost("/api/orch/queue/clear", async () =>
{
  try
  {
...
app.MapGet("/api/tasks/all", async (HttpContext ctx) =>
{
  SetNoStore(ctx.Response);
  try
  {
    ...
    lock (todoCacheLock)
    {
      if (tasksAllCache.Value.Count > 0 && (now - tasksAllCache.Timestamp).TotalMilliseconds < TasksAllCacheTtlMs)
      {
  ...
  catch (Exception error)
  {
    ...
    return Results.Json(new { error = "Failed to load tasks from OpenCode" }, statusCode: 502);
  }
});

app.MapPost("/api/tasks/{sessionId}/{taskId}/note", () => Results.Json(new { error = "Not implemented for OpenCode todos" }, statusCode: 501));
app.MapDelete("/api/tasks/{sessionId}/{taskId}", () => Results.Json(new { error = "Not implemented for OpenCode todos" }, statusCode: 501));
...
app.MapGet("/api/events", async (HttpContext ctx) =>
{
  ...
  await client.Send(new { type = "connected" });
...
{
  if (!int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var n)) return fallback;
  if (min.HasValue && n < min.Value) return min.Value;
  return n;
}
...
string FindWorkspaceRoot()
{
  ...
  foreach (var start in candidates)
  {
    ...
    for (var i = 0; i < 8; i++)
    {
      if (File.Exists(Path.Combine(current, "package.json"))) return current;
...
string? NormalizeDirectory(string? value)
{
  ...
  var s = value.Trim();
  if (s is "/" or "\\") return s;
  if (System.Text.RegularExpressions.Regex.IsMatch(s, "^[a-zA-Z]:[\\\\/]$")) return s;
  return s.TrimEnd('/', '\\');
}
...
string GetDirectoryCacheKey(string? value)
{
  return ToForwardSlashDirectory(value) ?? string.Empty;
}
...
{
  return session?["directory"]?.ToString()
    ?? session?["project"]?["worktree"]?.ToString();
}

string? GetProjectDisplayPath(JsonObject? session)
{
  return session?["project"]?["worktree"]?.ToString()
    ?? session?["projectWorktree"]?.ToString()
...
string? BuildOpenCodeSessionUrl(string sessionId, string? directory)
{
  ...
  if (string.IsNullOrWhiteSpace(dir)) return $"{baseUrl}/session/{Uri.EscapeDataString(sid)}";
...
List<JsonObject> ToArrayResponse(JsonNode? value)
{
  ...
  {
    if (obj["items"] is JsonArray items) return items.OfType<JsonObject>().ToList();
    if (obj["sessions"] is JsonArray sessions) return sessions.OfType<JsonObject>().ToList();
    if (obj["data"] is JsonArray data) return data.OfType<JsonObject>().ToList();
  }
...
void CollectSandboxDirectoryCandidates(JsonNode? value, List<string> output)
{
  ...
  switch (value)
  {
    case JsonValue v:
    {
      ...
      if (!string.IsNullOrWhiteSpace(dir)) output.Add(dir);
      break;
    }
    case JsonArray arr:
      foreach (var n in arr) CollectSandboxDirectoryCandidates(n, output);
      break;
    case JsonObject obj:
    {
      var maybePath = obj["directory"]?.ToString()
        ?? obj["path"]?.ToString()
        ?? obj["worktree"]?.ToString()
        ?? obj["root"]?.ToString();
...
(string? Worktree, List<string> Directories) GetProjectSearchDirectories(JsonObject? project)
{
  ...
  if (!string.IsNullOrWhiteSpace(worktree) && worktree is not "/" and not "\\") candidates.Add(worktree);
  ...
  {
    if (string.IsNullOrWhiteSpace(c) || c is "/" or "\\") continue;
...
async Task<List<JsonObject>> ListProjects()
{
  ...
  lock (projectsCacheLock)
  {
    if (projectsCache.Data.Count > 0 && (now - projectsCache.Timestamp).TotalMilliseconds < ProjectsCacheTtlMs)
    {
...
async Task<List<JsonObject>> ListSessionsForDirectory(string directory, string? projectWorktree, int limit)
{
  ...
  if (string.IsNullOrWhiteSpace(dirKey)) return [];
  ...
  foreach (var candidateDir in GetDirectoryVariants(dir))
  {
    try
    {
      var data = await OpenCodeFetch("/session", new OpenCodeRequest
      {
      ...
      var list = ToArrayResponse(data)
        ...
        .Select(s =>
        {
          ...
          if (!string.IsNullOrWhiteSpace(projectWorktree))
          {
            ...
            obj["project"] = new JsonObject { ["worktree"] = projectWorktree };
          }
          return obj;
        })
        .ToList();
      ...
      foreach (var session in list)
      {
        ...
        if (!mergedById.ContainsKey(sid)) mergedById[sid] = session;
      }
    }
    catch (Exception error)
    {
  ...
  if (!hadSuccess && lastError is not null) throw lastError;
...
async Task<List<JsonObject>> ListGlobalSessions(string limitParam)
{
  ...
  lock (sessionsCacheLock)
  {
    if (sessionsCache.Data.Count > 0 && (now - sessionsCache.Timestamp).TotalMilliseconds < SessionsCacheTtlMs)
    {
  ...
  foreach (var project in projects)
  {
    ...
    foreach (var directory in info.Directories)
    {
      ...
      if (string.IsNullOrWhiteSpace(key) || !seenDirectoryKeys.Add(key)) continue;
  ...
  if (!string.Equals(limitParam, "all", StringComparison.OrdinalIgnoreCase)) sessions = sessions.Take(limit).ToList();
...
{
  var dir = NormalizeDirectory(directory);
  if (string.IsNullOrWhiteSpace(dir)) return new Dictionary<string, JsonObject>(StringComparer.Ordinal);
  var dirKey = GetDirectoryCacheKey(dir);
  if (string.IsNullOrWhiteSpace(dirKey)) return new Dictionary<string, JsonObject>(StringComparer.Ordinal);
  ...
  foreach (var candidateDir in GetDirectoryVariants(dir))
  {
    try
    {
      var result = await OpenCodeFetch("/session/status", new OpenCodeRequest { Directory = candidateDir });
      var normalized = new Dictionary<string, JsonObject>(StringComparer.Ordinal);
      if (result is JsonObject obj)
      {
        foreach (var kv in obj)
        {
          if (kv.Value is JsonObject sObj) normalized[kv.Key] = sObj;
          else normalized[kv.Key] = new JsonObject { ["type"] = kv.Value?.ToString() };
        }
      }
      ...
      if (statusMap.Count == 0) statusMap = normalized;
    }
    catch
    {
    }
...
async Task<List<JsonObject>> GetTodosForSession(string sessionId, string? directory)
{
  ...
  var data = await OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}/todo", new OpenCodeRequest { Directory = directory });
...
{
  var key = $"{GetDirectoryCacheKey(directory)}::{sessionId}";
  if (statusOverride.TryGetValue(key, out var over) && (DateTimeOffset.UtcNow - over.Ts).TotalMilliseconds < 60_000)
  {
...
{
  if (IsRuntimeRunning(runtimeType)) return "in_progress";
  if (hasAssistantResponse == true) return "completed";
  if (hasAssistantResponse == false) return "pending";

  if (!DateTimeOffset.TryParse(modifiedAt, out var ts)) return "pending";
...
async Task<JsonObject?> FindSessionInfo(string sessionId)
{
  lock (sessionsCacheLock)
  {
    if (sessionsCache.ById.TryGetValue(sessionId, out var info)) return info;
  }
...
async Task<string?> ArchiveSessionOnOpenCode(string sessionId, string? directory)
{
  ...
  {
    () => OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}", new OpenCodeRequest
    {
      Method = "PATCH",
      Directory = directory,
      JsonBody = new JsonObject
      {
        ["time"] = new JsonObject { ["archived"] = nowUnixMs }
      }
    }),
    () => OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}", new OpenCodeRequest
    {
      Method = "PATCH",
      Directory = directory,
      JsonBody = new JsonObject { ["archived"] = true }
    }),
    () => OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}/archive", new OpenCodeRequest
    {
      Method = "POST",
      Directory = directory
    })
  ...
  foreach (var attempt in attempts)
  {
    ...
    {
      await attempt();
      var updated = await OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}", new OpenCodeRequest { Directory = directory });
      var archivedAt = updated?["time"]?["archived"]?.ToString();
      if (!string.IsNullOrWhiteSpace(archivedAt)) return archivedAt;
...
{
  if (depth > 5 || value is null) return string.Empty;
...
string ExtractAssistantMessageText(JsonObject? message)
{
  var candidates = new[]
  {
    message?["content"], message?["text"], message?["message"], message?["body"], message?["output"], message?["response"],
    message?["parts"], message?["data"], message?["info"]?["content"], message?["info"]?["text"]
  };
...
{
  var tail = await OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}/message", new OpenCodeRequest
  {
    Query = new Dictionary<string, string?> { ["limit"] = "400" }
  });
  var tailArr = (tail as JsonArray)?.OfType<JsonObject>().ToList() ?? [];
  var match = FindLastAssistantMessage(tailArr);
  if (match is not null) return match;
  if (tailArr.Count < 400) return null;
...
async Task<bool?> GetHasAssistantResponse(string sessionId)
{
  ...
  var task = assistantPresenceInFlight.GetOrAdd(sessionId, async _ =>
  {
    try
    {
      var tail = await OpenCodeFetch($"/session/{Uri.EscapeDataString(sessionId)}/message", new OpenCodeRequest
      {
        Query = new Dictionary<string, string?> { ["limit"] = "200" }
      });
      var tailArr = (tail as JsonArray)?.OfType<JsonObject>().ToList() ?? [];
      if (tailArr.Any(m => string.Equals(m["info"]?["role"]?.ToString(), "assistant", StringComparison.OrdinalIgnoreCase)))
        return true;

      if (tailArr.Count < 200) return false;
...
{
  if (!IsRuntimeRunning(runtimeType)) return todos;
  if (todos.Any(t => string.Equals(t["status"]?.ToString(), "in_progress", StringComparison.Ordinal))) return todos;
...
object? MapQueueItemToSessionSummary(QueueItemRecord item)
{
  ...
  if (queueState is not ("queued" or "dispatching")) return null;

  var titleParts = new List<string>();
  if (!string.IsNullOrWhiteSpace(item.IssueKey)) titleParts.Add(item.IssueKey);
  if (!string.IsNullOrWhiteSpace(item.Rule)) titleParts.Add(item.Rule);
  if (!string.IsNullOrWhiteSpace(item.Message)) titleParts.Add(item.Message);
  ...
  return new
  {
    ...
    createdAt,
    modifiedAt,
    runtimeStatus = new { type = runtimeType },
    status = "pending",
    ...
    try
    {
      ...
      while (!reader.EndOfStream && !cancellationToken.IsCancellationRequested)
      {
        ...
        if (line.Length == 0)
        {
          ...
          if (evt is not null) await HandleUpstreamEvent(evt);
          continue;
        }
...
async Task HandleUpstreamEvent(JsonNode evt)
{
  ...
  if (type == "todo.updated")
  {
    ...
    if (!string.IsNullOrWhiteSpace(sessionId))
    {
      ...
      await sseHub.Broadcast(new { type = "update", sessionId });
    }
    else
    {
      InvalidateAllCaches();
  ...
  if (type == "session.status")
  {
    ...
    if (!string.IsNullOrWhiteSpace(sessionId) && !string.IsNullOrWhiteSpace(statusType))
    {
...
internal sealed record LastAssistantMessage(string? Message, string? CreatedAt);

internal sealed class TimestampedValue<T>
{
...
internal sealed class ProjectsCache
{
  public DateTimeOffset Timestamp { get; set; } = DateTimeOffset.MinValue;
  public List<JsonObject> Data { get; set; } = [];
}
...
internal sealed class SseClient
{
  ...
  public void Start()
  {
    Completion = Task.Run(async () =>
    {
      try
      {
        while (!_token.IsCancellationRequested && await _queue.Reader.WaitToReadAsync(_token))
        {
