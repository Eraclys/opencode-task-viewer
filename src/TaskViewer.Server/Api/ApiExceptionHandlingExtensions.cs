...
public static class ApiExceptionHandlingExtensions
{
        ...
        Func<Exception, ApiErrorResult> mapError)
    {
        return builder.AddEndpointFilterFactory(
            (_, next) => async invocationContext =>
            {
                try
                {
                ...
                catch (Exception error)
                {
                    ...
                    var logger = loggerFactory.CreateLogger("TaskViewer.Server.Api");
                    logger.LogError(error, "{Message}", logMessage);

                    var mapped = mapError(error);
                    return Results.Json(new ErrorResponseDto { Error = mapped.Message }, statusCode: mapped.StatusCode);
                }
            });
    }
}
