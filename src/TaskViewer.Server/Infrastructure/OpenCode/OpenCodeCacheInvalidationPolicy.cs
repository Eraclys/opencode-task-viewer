...
sealed class OpenCodeCacheInvalidationPolicy
{
    public OpenCodeCacheInvalidationDecision Decide(OpenCodeEventEnvelope parsed)
    {
        if (parsed.Type == "todo.updated")
        {
            if (!string.IsNullOrWhiteSpace(parsed.SessionId))
                return new OpenCodeCacheInvalidationDecision(true, false, false, false, true, parsed.SessionId);

            return new OpenCodeCacheInvalidationDecision(false, true, false, false, true, null);
        }
        ...
        if (parsed.Type.StartsWith("message.", StringComparison.Ordinal))
            return new OpenCodeCacheInvalidationDecision(false, false, true, false, true, null, ClearAssistantPresence: true);
