...
sealed class OpenCodeSessionSearchService
{
    readonly OpenCodeApiClient _openCodeApiClient;
    ...
    {
        _openCodeApiClient = openCodeApiClient;
        _cacheCoordinator = cacheCoordinator;
    }

    public string? BuildOpenCodeSessionUrl(string sessionId, string? directory) => _openCodeApiClient.BuildSessionUrl(sessionId, directory);
    ...
    public async Task<Dictionary<string, SessionRuntimeStatus>> GetStatusMapForDirectoryAsync(string? directory)
    {
        ...
        foreach (var candidateDirectory in DirectoryPath.GetVariants(normalizedDirectory))
        {
            try
            {
                var result = await _openCodeApiClient.ReadWorkingStatusMapAsync(candidateDirectory);
    ...
    public async Task<List<SessionTodoDto>> GetTodosForSessionAsync(string sessionId, string? directory)
    {
        ...
        var rawTodos = await _openCodeApiClient.ReadTodosAsync(sessionId, directory);
    ...
    {
        return await _openCodeApiClient.ArchiveSessionAsync(sessionId, directory);
    }

    public async Task<LastAssistantMessage?> GetLastAssistantMessageAsync(string sessionId)
    {
        var tailMessages = await _openCodeApiClient.ReadMessagesAsync(sessionId, 400);
        ...
        var allMessages = await _openCodeApiClient.ReadMessagesAsync(sessionId);
    ...
    public async Task<bool?> GetHasAssistantResponseAsync(string sessionId)
    {
        if (_cacheCoordinator.TryGetFreshAssistantPresence(sessionId, out var cachedPresence))
            return cachedPresence;

        var task = _cacheCoordinator.GetOrAddAssistantPresenceInFlight(
            sessionId,
            async _ =>
            {
                try
                {
                    var tailMessages = await _openCodeApiClient.ReadMessagesAsync(sessionId, 200);

                    if (tailMessages.Any(message => string.Equals(message.Role, "assistant", StringComparison.Ordinal)))
                        return true;

                    if (tailMessages.Count < 200)
                        return false;

                    var allMessages = await _openCodeApiClient.ReadMessagesAsync(sessionId);
                    return allMessages.Any(message => string.Equals(message.Role, "assistant", StringComparison.Ordinal));
                }
                catch
                {
                    return null;
        ...
        _cacheCoordinator.CompleteAssistantPresenceLookup(sessionId, result, DateTimeOffset.UtcNow);
    ...
    async Task<List<OpenCodeProjectTransport>> ListProjectsAsync()
    {
        ...
        var projects = await _openCodeApiClient.ReadProjectsAsync();
    ...
    async Task<List<OpenCodeSessionDto>> ListSessionsForDirectoryAsync(string directory, string? projectWorktree, int limit, int maxSessionsPerProject)
    {
        ...
        foreach (var candidateDirectory in DirectoryPath.GetVariants(normalizedDirectory))
        {
            try
            {
                var sessionsForDirectory = await _openCodeApiClient.ReadSessionsAsync(candidateDirectory, perRequestLimit);
                hadSuccess = true;
