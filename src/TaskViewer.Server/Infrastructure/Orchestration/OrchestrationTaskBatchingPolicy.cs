...
static class OrchestrationTaskBatchingPolicy
{
    ...
    public static string BuildLockKey(MappingRecord mapping, string? path)
    {
        return $"{mapping.SonarProjectKey}::{mapping.Branch ?? string.Empty}::{NormalizePath(path)}";
    }
    ...
    public static string BuildRepresentativeMessage(IReadOnlyList<NormalizedIssue> issues, string? path, string? rule)
    {
        ...
        if (!string.IsNullOrWhiteSpace(representative) && issues.Count == 1)
