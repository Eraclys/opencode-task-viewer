...
public interface IMappingRepository
{
    ...
    Task<MappingRecord> UpsertMapping(int? id, string sonarProjectKey, string directory, string? branch, bool enabled, string now);
    Task<JsonObject?> GetInstructionProfile(int mappingId, string issueType);
    Task<JsonObject> UpsertInstructionProfile(int mappingId, string issueType, string instructions, string now);
