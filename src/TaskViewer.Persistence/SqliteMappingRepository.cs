...
public sealed class SqliteMappingRepository : IMappingRepository
{
    ...
    public async Task<List<MappingRecord>> ListMappings(CancellationToken cancellationToken = default)
    {
        ...
        var rows = await conn.QueryAsync<MappingRow>(Cmd(@"
SELECT
    id AS Id,
...
ORDER BY sonar_project_key COLLATE NOCASE ASC", cancellationToken: cancellationToken));
    ...
    {
        using var conn = _openConnection();
        var row = await conn.QuerySingleOrDefaultAsync<MappingRow>(Cmd(@"
SELECT
    id AS Id,
    sonar_project_key AS SonarProjectKey,
    directory AS Directory,
...
FROM project_mappings
WHERE id = @Id
LIMIT 1", new { Id = id }, cancellationToken: cancellationToken));
    ...
    public async Task<bool> DeleteMapping(int id, CancellationToken cancellationToken = default)
    {
        ...
        var deleted = await conn.ExecuteAsync(Cmd(@"
DELETE FROM project_mappings
WHERE id = @Id", new { Id = id }, tx, cancellationToken));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (id.HasValue &&
            id.Value > 0)
        {
            var updated = await conn.ExecuteAsync(Cmd(@"
...
FROM project_mappings
WHERE id = @Id
LIMIT 1", new { Id = id.Value }, cancellationToken: cancellationToken));
...
ON CONFLICT(sonar_project_key) DO UPDATE SET
  ...
  updated_at = excluded.updated_at", new
        ...
        var insertedRow = await conn.QuerySingleOrDefaultAsync<MappingRow>(Cmd(@"
SELECT
    id AS Id,
    sonar_project_key AS SonarProjectKey,
    directory AS Directory,
...
LIMIT 1", new { SonarProjectKey = sonarProjectKey }, cancellationToken: cancellationToken));
    ...
    {
        using var conn = _openConnection();
        var row = await conn.QuerySingleOrDefaultAsync<InstructionProfileRow>(Cmd(@"
SELECT
    id AS Id,
...
LIMIT 1", new { MappingId = mappingId, IssueType = issueType }, cancellationToken: cancellationToken));
...
LIMIT 1", new { MappingId = mappingId, IssueType = issueType }, cancellationToken: cancellationToken));
    ...
    {
        using var conn = _openConnection();
        var directories = await conn.QueryAsync<string>(Cmd(@"
SELECT directory
FROM project_mappings
WHERE enabled = 1", cancellationToken: cancellationToken));
