...
This repo is an ASP.NET Core server plus a single-page UI served from `src/TaskViewer.Server/wwwroot/index.html` (no build step).
...
- `src/TaskViewer.Server/wwwroot/index.html`: UI (inline CSS + inline JS; no bundler)
...
- Keep response shapes stable; UI in `src/TaskViewer.Server/wwwroot/index.html` depends on exact fields.

### UI (`src/TaskViewer.Server/wwwroot/index.html`)
...
- If you change `/api/*` response shapes in `src/TaskViewer.Server/Program.cs`, update `src/TaskViewer.Server/wwwroot/index.html` and E2E tests.
